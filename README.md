...
Investors considering these loans will favor protecting their principal over interest income, which means we will need to minimize false positives.  Precision is the most appropriate metric to use our model evaluation.
...
The risk appetite of each investor will determine which model we use to help them with their loan investment decision:
...
Optimizing loan portfolio performance can be very complex and need to take into account many aspects including credit risk management, diversification, and other investor preferences.  We have only scratched the surface in this project.
